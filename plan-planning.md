...
SSH Key can be generated by using any method. When generating a key pair, make sure that the **passphrase is empty(must not be password encrypted)** and key format is `RSA`.
