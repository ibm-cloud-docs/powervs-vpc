...
- Support all private PowerVS CIDRs in ACL rule;
- Import any number of PowerVS images , and in parallel.
- clean_default_sg_acl included in the landing zone preset to clean the default sg rule for new deployments
- Upgrade to ibm-sles-15-4-amd64-sap-applications-6 & ibm-redhat-8-6-amd64-sap-applications-4 on intel VSI landing zone for new deployments. Will not affect the previous DA version
- All DAs now use terraform-ibm-modules/powervs-workspace/ibm module to create PowerVS Workspace.
- Change name of DCs in Drop down for DAs and make it more detailed for PER
...
- Faster parallel deployments of landing zone and PowerVS infrastructure
- Fix: dns settings on landing zone vsi (upgraded to 1.1.4 version ansible collection ibm.power_linux_sap collection)

Breaking Change :warning:
- :warning: Please backup the data on 10.20.10.4 private-svs-1 vsi under the /nfs directory before upgrading to this.
